...
When `memory_model.active: true` in `teams.yaml`, each turn's input + output is summarized into `.pi_memory/project_memory.md`. On every turn, the orchestrator's system prompt includes a **Project Memory** section that points to that file, so it knows where accumulated context lives and can read it (via `read`) when prior decisions, facts, or user preferences are relevant. Sections maintained: **Design Decisions, Facts, User Taste, User Suggestions**. Merely setting `model` does **not** enable memory — `active: true` is required.
